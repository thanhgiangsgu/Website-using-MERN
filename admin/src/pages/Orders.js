...
function Row(props) {
  ...
  const handleApprovalOrder = (dataOrder) => {
    const data = {
      order_id: dataOrder.order_id,
      order_status: "Đang giao hàng",
    }
    if (dataOrder.order_status == "Đang xử lý")
    {
  ...
  const handleDeleteOrder = async (id) => {
    await Axios.delete(`http://localhost:3002/orderdetail/delete-product-by-order-id/${id}`)
    Axios.delete(`http://localhost:3002/order/delete-order-by-id/${id}`)
    window.location.reload(); 
  }
      ...
      <TableRow sx={{ '& > *': { borderBottom: 'unset' } }}>
        ...
        <TableCell align="center">
          ...
          <Button
            ...
            sx={{ mt: 3, mb: 2 }}
            fullWidth
            disabled={row.order_status == "Đang xử lý" ? false : true }
            onClick={() => handleApprovalOrder(row)}
          >Duyệt</Button>
          <Button
            type="submit"
            variant="contained"
