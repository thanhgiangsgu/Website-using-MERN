...
const EnhancedTableToolbar = (props) => {
  ...
  const {numSelected, selectedItem , getDataCategory} = props;
  const [showadd, setShowAdd] = useState(false)
  const [showedit, setShowEdit] = useState(false)
  const [info, setInfo] = useState(initialState)
  const {category_id, category_name} = info
  ...
  const handleDelete = (event) =>
  {
    {selectedItem.map(item => 
      Axios.delete(`http://localhost:3002/category/delete-category/${item}`)
      )}
      getDataCategory();
      numSelected = 0
  }
  ...
  const handleSubmitAddData = async (event) =>
  {
    ...
    const res = await Axios.post(`http://localhost:3002/category/check-category/${info.category_id}`)
    if (res.data.check == "false")
    {
      Axios.post(`http://localhost:3002/category/add-category/`, info)
  ...
  const handleSubmitEditData = async (event) =>
  {
    ...
    //const res = Axios.post(`http://localhost:3002/account/check-username`, info.username)
      ...
      Axios.patch(`http://localhost:3002/category/update-category/`, info)
  ...
  return (
    <>
    <h1 style={{}}>QUẢN LÍ KHÁCH HÀNG</h1>
    <Toolbar
      sx={{
        pl: { sm: 2 },
        pr: { xs: 1, sm: 1 },
...
export default function EnhancedTable() {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelected = rows.map((n) => n.category_id);
  ...
  useEffect(() => {
   ...
   const getDataCategory = React.useCallback(() => {
  ...
  return (
    ...
    <Box sx={{ width: '100%' }}>
      <Paper sx={{ width: '100%', mb: 2 }}>
        ...
        <EnhancedTableToolbar numSelected={selected.length} selectedItem={selected} getDataCategory={getDataCategory} />
        <TableContainer>
          <Table
            sx={{ minWidth: 750 }}
